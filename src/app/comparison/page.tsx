...
const RADAR_ORIGINS = [
  { name: 'Nepal', color: '#D4A55A', opacity: 0.3 },
  { name: 'Ethiopia', color: '#6B7F7E', opacity: 0.2 },
...
function AnimatedBar({
  ...
  return (
    <div ref={ref} className="flex items-center gap-2 w-full">
      <div className="flex-1 bg-[#D9DFDC] rounded-full h-2 overflow-hidden">
        <motion.div
          className="h-full rounded-full"
          style={{ backgroundColor: isNepal ? '#D4A55A' : '#6B7F7E' }}
          initial={{ width: 0 }}
      ...
      <span
        ...
        style={{ color: isNepal ? '#D4A55A' : '#6B7F7E' }}
      >
        {value}
      </span>
    </div>
  ...
  return (
    <div className="space-y-10">
      {filteredFactors.map((factor) => (
        <div key={factor.key} className="rounded-2xl overflow-hidden border border-[#D9DFDC]">
          {/* Factor header */}
          <div className="px-4 sm:px-6 py-4 bg-[#1F4D4F]">
            <h3 className="font-serif text-lg text-[#F6F1E9] font-semibold">{factor.label}</h3>
          </div>
          {/* Origin bars */}
          <div className="divide-y divide-[#D9DFDC]">
              ...
              >
                <div className="w-16 sm:w-24 shrink-0">
                  <p
                    ...
                    style={{ color: origin.isNepal ? '#D4A55A' : '#1F4D4F' }}
                  >
                    {origin.name}
                  </p>
                  <p className="font-sans text-xs text-[#6B7F7E] hidden sm:block">{origin.country}</p>
                </div>
...
function AltitudeRow() {
  ...
  return (
    <div ref={ref} className="rounded-2xl overflow-hidden border border-[#D9DFDC] mb-10">
      <div className="px-4 sm:px-6 py-4 bg-[#1F4D4F]">
        <h3 className="font-serif text-lg text-[#F6F1E9] font-semibold">Altitude (m)</h3>
      </div>
      <div className="divide-y divide-[#D9DFDC]">
        {ORIGINS.map((origin, oi) => {
            ...
            >
              ...
              <div className="flex-1 flex items-center gap-2">
                <div className="flex-1 bg-[#D9DFDC] rounded-full h-2 overflow-hidden">
                  <motion.div
                    className="h-full rounded-full"
                    style={{ backgroundColor: origin.isNepal ? '#D4A55A' : '#6B7F7E' }}
                    initial={{ width: 0 }}
                    animate={inView ? { width: `${pct}%` } : { width: 0 }}
                ...
                <span
                  ...
                  style={{ color: origin.isNepal ? '#D4A55A' : '#6B7F7E' }}
                >
                  {origin.altitude}m
                </span>
              </div>
      ...
      <div className="flex flex-wrap justify-center gap-6 mt-6">
        {radarOriginsData.map((ro) => (
          <div key={ro.name} className="flex items-center gap-2">
            ...
            />
            <span className="font-sans text-sm text-[#1F4D4F] font-medium">{ro.name}</span>
          </div>
        ))}
      </div>
    </div>
...
function DifferentiatorCard({
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl p-5 sm:p-8 border border-[#D9DFDC] flex flex-col gap-4"
    >
      <div
        className="w-12 h-12 rounded-full flex items-center justify-center text-xl"
        style={{ backgroundColor: '#F6F1E9', color: '#D4A55A' }}
      >
        {item.icon}
      </div>
      <h3 className="font-serif text-xl text-[#1F4D4F] font-semibold leading-snug">
        {item.title}
      </h3>
      <p className="font-sans text-[#6B7F7E] text-sm leading-relaxed">{item.description}</p>
    </motion.div>
  );
}
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage:
              'radial-gradient(circle at 20% 50%, #D4A55A 0%, transparent 60%), radial-gradient(circle at 80% 20%, #6B7F7E 0%, transparent 50%)',
          }}
        />
        ...
        <div className="relative z-10 site-container w-full">
          <motion.p
            ...
            className="font-sans text-[#D4A55A] text-sm font-semibold tracking-[0.2em] uppercase mb-6"
          >
            Origin Comparison
          </motion.p>
          <motion.h1
            ...
            className="font-serif text-4xl sm:text-5xl md:text-7xl lg:text-8xl text-[#F6F1E9] font-bold leading-[0.95] mb-8"
          >
            The World&apos;s Best,
            <br />
            <em className="text-[#D4A55A]">Side by Side</em>
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={heroInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.7, delay: 0.25 }}
            className="font-sans text-[#D9DFDC] text-lg max-w-2xl leading-relaxed"
          >
      ...
      <section className="py-12 sm:py-16 md:py-20 site-container">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
          <div>
            ...
            <h2 className="font-serif text-4xl md:text-5xl text-[#1F4D4F] font-bold leading-tight mb-6">
              Nepal Has Arrived in the World&apos;s Top Tier
            </h2>
          </div>
          <div>
            <p className="font-sans text-[#6B7F7E] text-base leading-relaxed mb-4">
      ...
      <section className="site-container mb-6 md:mb-8">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-sans text-sm text-[#6B7F7E] font-medium mr-2">Filter by:</span>
          <button
            onClick={() => setActiveFilter(null)}
            className="px-4 py-1.5 rounded-full text-sm font-sans font-medium transition-all duration-200"
            style={{
              backgroundColor: activeFilter === null ? '#1F4D4F' : '#D9DFDC',
              color: activeFilter === null ? '#F6F1E9' : '#1F4D4F',
            }}
          >
            All Factors
          </button>
          {FACTORS.map((f) => (
            <button
              ...
              className="px-4 py-1.5 rounded-full text-sm font-sans font-medium transition-all duration-200"
              style={{
                backgroundColor: activeFilter === f.key ? '#D4A55A' : '#D9DFDC',
                color: activeFilter === f.key ? '#F6F1E9' : '#1F4D4F',
              }}
            >
              {f.label}
            </button>
        ...
        <div className="site-container">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            <div>
              ...
              <p className="font-sans text-[#D9DFDC] text-base leading-relaxed mb-4">
            ...
            <div className="bg-[#F6F1E9] rounded-3xl p-4 sm:p-8">
              <RadarChart />
            </div>
          </div>
        </div>
      ...
      >
        <div className="site-container" style={{maxWidth:"680px"}}>
          ...
          <p className="font-sans text-[#F6F1E9] text-base leading-relaxed mb-10 opacity-90">
