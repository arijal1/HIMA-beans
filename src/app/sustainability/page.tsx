...
function CommitmentBlock({
  ...
  return (
    <section
      style={{
        backgroundColor: section.dark ? '#222222' : '#F5EFE6',
      }}
    >
      <div
        ref={ref}
      ...
      >
        ...
        >
          ...
          <h2
            ...
            style={{ color: section.dark ? '#F5EFE6' : '#3B2A21' }}
          >
            {section.title}
          </h2>
          <p
          ...
          <div className="space-y-4">
            {section.body.map((para, i) => (
              <p
                ...
                style={{ color: section.dark ? '#E6D8C9' : '#8C8477' }}
              >
                {para}
              </p>
            ))}
...
function FarmerCard({ farmer, index }: { farmer: FarmerProfile; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-3xl overflow-hidden border border-[#E6D8C9]"
    >
      {/* Avatar banner */}
      <div
        className="h-32 flex items-end px-8 pb-0 relative"
        style={{
          background: 'linear-gradient(135deg, #3B2A21 0%, #B08D57 100%)',
        }}
      >
        <div
      ...
      <div className="px-8 pt-14 pb-8">
        <h3 className="font-serif text-2xl text-[#3B2A21] font-semibold mb-1">{farmer.name}</h3>
        <p className="font-sans text-sm text-[#B08D57] font-semibold mb-0.5">{farmer.location}</p>
        <p className="font-sans text-xs text-[#8C8477] mb-6">{farmer.tenure}</p>

        <blockquote className="font-serif text-base italic text-[#3B2A21] leading-relaxed mb-6 border-l-2 border-[#B08D57] pl-4">
          {farmer.quote}
        </blockquote>

        <p className="font-sans text-sm text-[#8C8477] leading-relaxed">{farmer.detail}</p>
      </div>
    </motion.div>
  );
}
...
function ImpactCard({ stat, index }: { stat: ImpactStat; index: number }) {
  ...
  return (
    ...
    >
      ...
      <p className="font-sans text-sm text-[#8C8477]">{stat.description}</p>
    </motion.div>
  );
}
...
function CertBadge({ cert, index }: { cert: Certification; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center gap-3 p-8 rounded-2xl border-2 border-[#E6D8C9] bg-white"
    >
      <div
      ...
      <p className="font-serif text-lg text-[#3B2A21] font-semibold text-center">{cert.name}</p>
      <p className="font-sans text-xs text-[#8C8477] text-center">{cert.body}</p>
    </motion.div>
  );
}
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse at 50% 100%, #3B2A21 0%, transparent 60%), radial-gradient(ellipse at 80% 30%, #8C8477 0%, transparent 50%), radial-gradient(ellipse at 20% 60%, #3B2A21 0%, transparent 40%)',
          }}
        />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 w-full">
          ...
          >
            Our Earth
            <br />
            <em className="text-[#B08D57]">Promise</em>
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={heroInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.7, delay: 0.3 }}
            className="font-sans text-[#E6D8C9] text-lg max-w-xl leading-relaxed"
          >
